...
class Level:
    ...
    def __init__(self):
        ...
        # sprite setup
        self.enemys, self.villagers, self.bosses=self.create_map()
                    ...
                    if col != '-1': #Case where the tile is not empty
                        ...
                        if style == 'entities':
                            if col == '4': #Player
                                ...
                                villagers.append(Villager((x,y), [self.visible_sprites, self.obstacle_sprites], image, self.player, speech))
                            elif col == '8':
                                ...
                                bosses.append(Boss(monsters_name,(x,y),
                                    [self.visible_sprites,self.attackable_sprites],
                                    self.obstacle_sprites,
                                    self.damage_player,
                                    self.create_particles,
                                     self.create_boss_magic))
        return enemys, villagers, bosses
    ...
    def run(self):
        ...
        for enemy in self.enemys:
            enemy.death_sound.set_volume(0.4*menu.sfx_vol/4)
            enemy.hit_sound.set_volume(0.4*menu.sfx_vol/4)
            enemy.attack_sound.set_volume(0.4*menu.sfx_vol/4)
        for villager in self.villagers:
            villager.voice.set_volume(menu.sfx_vol/10)
        for boss in self.bosses:
            boss.death_sound.set_volume(0.4*menu.sfx_vol/4)
            boss.hit_sound.set_volume(0.4*menu.sfx_vol/4)
            boss.attack_sound.set_volume(0.4*menu.sfx_vol/4)
